...
CREATE TABLE auth.memberships (
  ...
  role VARCHAR(50) NOT NULL DEFAULT 'member', -- owner, admin, member, coach, student
  created_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE TABLE coaching.metrics (
  ...
  session_id UUID REFERENCES coaching.sessions(id) ON DELETE CASCADE,
  metric_type VARCHAR(50) NOT NULL, -- wpm, pitch, volume, pauses
...
CREATE TABLE coaching.fluency (
  ...
  ttr_ratio DECIMAL(5,4), -- Type-Token Ratio
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE coaching.drills (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  session_id UUID REFERENCES coaching.sessions(id) ON DELETE CASCADE,
  drill_type VARCHAR(50) NOT NULL, -- minimal_pairs, pacing, shadowing
...
-- RLS policies for coaching tables (basic - will be refined)
CREATE POLICY "Users can access their organization's data" ON coaching.projects
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM auth.memberships 
